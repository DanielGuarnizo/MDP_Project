...
void top_level(DTYPE *dram_in_b0, DTYPE *dram_in_b1, DTYPE *dram_w_b0, DTYPE *dram_w_b1, DTYPE *dram_out_b0, DTYPE *dram_out_b1, DTYPE *dram_out_b2, DTYPE *dram_out_b3, DTYPE *dram_out_b4, DTYPE *dram_out_b5, DTYPE *dram_out_b6, DTYPE *dram_out_b7, DTYPE *dram_out_b8, DTYPE *dram_out_b9, DTYPE *dram_out_b10, DTYPE *dram_out_b11, DTYPE *dram_out_b12, DTYPE *dram_out_b13, DTYPE *dram_out_b14, DTYPE *dram_out_b15)
{
    ...
    for (int gb_0 = 0; gb_0 < 4; ++gb_0) {
      ...
      // w_tile[4][2][2]: level-indexed, Q absent (weight is Q-independent)
      DTYPE w_tile[4][2][2];
      #pragma GCC unroll 4
      for (int sarows_0 = 0; sarows_0 < 4; ++sarows_0) {
        #pragma GCC unroll 2
        for (int sarows_1 = 0; sarows_1 < 2; ++sarows_1) {
          ...
          for (int sacols_1 = 0; sacols_1 < 2; ++sacols_1) {
            ...
            int w_idx = ((sarows_1*2 + sacols_1) * ((C + in_banks - 1) / in_banks) + c_blk) * (R * S) + r * S + 0;
            w_tile[sarows_0][sarows_1][sacols_1] = (c_bank==0) ? dram_w_b0[w_idx] : dram_w_b1[w_idx];
          }  // sacols_1 (preload)
        }  // sarows_1 (preload)
      }  // sarows_0 (preload)

      // ---- Phase 2a: multiply — 64 independent products ----
      // p[4][2][4][2]: GCC SROA → 64 scalar float regs
      DTYPE p[4][2][4][2];
      int q_base = 0;
      #pragma GCC unroll 4
      ...
      // ---- Phase 2b: accumulate — 64 independent, no RAW chain ----
      #pragma GCC unroll 4
      for (int sarows_0 = 0; sarows_0 < 4; ++sarows_0) {
        #pragma GCC unroll 2
        for (int sarows_1 = 0; sarows_1 < 2; ++sarows_1) {
      ...
      // ---- reduction: 64 acc → 16 outputs (4 inputs each) ----
      DTYPE reduced[2][4][2];
      #pragma GCC unroll 2
      for (int sarows_1 = 0; sarows_1 < 2; ++sarows_1) {
        #pragma GCC unroll 4
        for (int sacols_0 = 0; sacols_0 < 4; ++sacols_0) {
          #pragma GCC unroll 2
          for (int sacols_1 = 0; sacols_1 < 2; ++sacols_1) {
            DTYPE _t0_0 = acc[0][sarows_1][sacols_0][sacols_1] + acc[1][sarows_1][sacols_0][sacols_1];
            DTYPE _t0_1 = acc[2][sarows_1][sacols_0][sacols_1] + acc[3][sarows_1][sacols_0][sacols_1];
            DTYPE _t1_0 = _t0_0 + _t0_1;
            reduced[sarows_1][sacols_0][sacols_1] = _t1_0;
          }  // sacols_1 (reduction)
        }  // sacols_0 (reduction)
      }  // sarows_1 (reduction)
