...
int main() {
    ...
    DTYPE *dram_w_b0  = (DTYPE*)malloc(4  * sizeof(DTYPE));
    DTYPE *dram_w_b1  = (DTYPE*)malloc(4  * sizeof(DTYPE));
    DTYPE *dram_in_b0 = (DTYPE*)malloc(4 * sizeof(DTYPE));
    DTYPE *dram_in_b1 = (DTYPE*)malloc(4 * sizeof(DTYPE));
    ...
    for (size_t i = 0; i < 4;  ++i) { dram_w_b0[i] = 0.0f; dram_w_b1[i] = 0.0f; }
    for (size_t i = 0; i < 4; ++i) { dram_in_b0[i] = 0.0f; dram_in_b1[i] = 0.0f; }
    ...
    for (int m = 0; m < 4; ++m) {
      for (int k = 0; k < 1; ++k) {
        ...
        DTYPE v = A_full[m * 1 + k];
        if (k_bank == 0) dram_w_b0[m * 1 + k_blk] = v;
        else             dram_w_b1[m * 1 + k_blk] = v;
      }
    }
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 4 * sizeof(DTYPE));
    m_param_alloc(1, 4 * sizeof(DTYPE));
    m_param_alloc(2, 4 * sizeof(DTYPE));
    m_param_alloc(3, 4 * sizeof(DTYPE));
    ...
    top_level(dram_w_b0, dram_w_b1, dram_in_b0, dram_in_b1, dram_out_b0, dram_out_b1);
