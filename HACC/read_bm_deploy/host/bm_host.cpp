// bm_host.cpp — AXI latency benchmark host for read_bm_krnl
...
int main(int argc, char* argv[]) {
    const char* xclbin_path = (argc > 1) ? argv[1] : "read_bm_krnl.xclbin";

    auto device = xrt::device(0);
    auto uuid   = device.load_xclbin(xclbin_path);
    auto krnl   = xrt::kernel(device, uuid, "read_bm_krnl", xrt::kernel::cu_access_mode::exclusive);
    ...
    for (int n : n_list) {
        ...
        krnl.write_register(0x014,
            (uint32_t)(buf_output_p0.address() & 0xFFFFFFFF));
        krnl.write_register(0x018, (uint32_t)n);
