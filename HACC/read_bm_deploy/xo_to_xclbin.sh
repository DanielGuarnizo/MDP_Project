#!/usr/bin/env bash
# xo_to_xclbin.sh — link read_bm_krnl.xo into a hw xclbin for Alveo U55C
set -eo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XO="${SCRIPT_DIR}/xo/read_bm_krnl.xo"
...
v++ -t hw \
  ...
  --connectivity.sp read_bm_krnl_1.m_axi_gmem_0:HBM[0] \
  -o "${SCRIPT_DIR}/xo/read_bm_krnl.xclbin"
