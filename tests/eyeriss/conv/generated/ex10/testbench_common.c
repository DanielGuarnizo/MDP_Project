...
int main() {
    ...
    // Input ports
    DTYPE *dram_input_p0  = (DTYPE*)malloc(216 * sizeof(DTYPE));
    DTYPE *dram_input_p1  = (DTYPE*)malloc(216 * sizeof(DTYPE));
    ...
    // Weight ports
    DTYPE *dram_weight_p0 = (DTYPE*)malloc(288 * sizeof(DTYPE));
    DTYPE *dram_weight_p1 = (DTYPE*)malloc(288 * sizeof(DTYPE));
    ...
    // Zero all ports
    for (size_t i = 0; i < 216; ++i) dram_input_p0[i] = 0.0f;
    for (size_t i = 0; i < 216; ++i) dram_input_p1[i] = 0.0f;
    for (size_t i = 0; i < 288; ++i) dram_weight_p0[i] = 0.0f;
    for (size_t i = 0; i < 288; ++i) dram_weight_p1[i] = 0.0f;
    ...
    for (int m = 0; m < 4; ++m) {
      for (int c = 0; c < 8; ++c) {
        ...
        for (int r = 0; r < 3; ++r)
          for (int s = 0; s < 6; ++s) {
            int full_idx = m*8*3*6 + c*3*6 + r*6 + s;
            int b_idx    = (m*4 + blk)*3*6 + r*6 + s;
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 216 * sizeof(DTYPE));
    m_param_alloc(1, 216 * sizeof(DTYPE));
    m_param_alloc(2, 288 * sizeof(DTYPE));
    m_param_alloc(3, 288 * sizeof(DTYPE));
