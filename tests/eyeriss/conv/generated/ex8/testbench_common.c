...
int main() {
    ...
    // Input ports
    DTYPE *dram_input_p0  = (DTYPE*)malloc(72 * sizeof(DTYPE));
    DTYPE *dram_input_p1  = (DTYPE*)malloc(72 * sizeof(DTYPE));
    DTYPE *dram_input_p2  = (DTYPE*)malloc(72 * sizeof(DTYPE));
    DTYPE *dram_input_p3  = (DTYPE*)malloc(72 * sizeof(DTYPE));
    ...
    // Zero all ports
    for (size_t i = 0; i < 72; ++i) dram_input_p0[i] = 0.0f;
    for (size_t i = 0; i < 72; ++i) dram_input_p1[i] = 0.0f;
    for (size_t i = 0; i < 72; ++i) dram_input_p2[i] = 0.0f;
    for (size_t i = 0; i < 72; ++i) dram_input_p3[i] = 0.0f;
    ...
    for (int m = 0; m < 4; ++m) {
      for (int c = 0; c < 8; ++c) {
        ...
        for (int r = 0; r < 3; ++r)
          for (int s = 0; s < 3; ++s) {
            int full_idx = m*8*3*3 + c*3*3 + r*3 + s;
