...
void top_level(DTYPE *dram_in_b0, DTYPE *dram_in_b1, DTYPE *dram_w_b0, DTYPE *dram_w_b1, DTYPE *dram_out_b0, DTYPE *dram_out_b1, DTYPE *dram_out_b2, DTYPE *dram_out_b3, DTYPE *dram_out_b4, DTYPE *dram_out_b5, DTYPE *dram_out_b6, DTYPE *dram_out_b7)
{
    ...
    // 96 PE accumulators: acc[3][4][2][4]
    DTYPE acc[3][4][2][4];
      ...
      for (int gb_1 = 0; gb_1 < 4; ++gb_1) {
        ...
        for (int c_seq = 0; c_seq < 2; ++c_seq) {
          ...
          for (int r = 0; r < 3; ++r) {
            ...
            // w_tile[3][4][4]: level-indexed, Q absent (weight is Q-independent)
            DTYPE w_tile[3][4][4];
            #pragma GCC unroll 3
            for (int sarows_0 = 0; sarows_0 < 3; ++sarows_0) {
              #pragma GCC unroll 4
            ...
            // p[3][4][2][4]: GCC SROA → 96 scalar float regs
            DTYPE p[3][4][2][4];
            int q_base = gb_0 * 2;
            #pragma GCC unroll 3
