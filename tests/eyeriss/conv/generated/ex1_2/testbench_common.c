...
int main() {
    ...
    DTYPE *dram_input_p3  = (DTYPE*)malloc(36 * sizeof(DTYPE));
    ...
    DTYPE *dram_weight_p3 = (DTYPE*)malloc(36 * sizeof(DTYPE));
    ...
    // Output ports
    DTYPE *dram_output_p0 = (DTYPE*)malloc(16 * sizeof(DTYPE));
    if (!dram_output_p0) { printf("Output port 0 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p1 = (DTYPE*)malloc(16 * sizeof(DTYPE));
    if (!dram_output_p1) { printf("Output port 1 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p2 = (DTYPE*)malloc(16 * sizeof(DTYPE));
    if (!dram_output_p2) { printf("Output port 2 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p3 = (DTYPE*)malloc(16 * sizeof(DTYPE));
    ...
    for (size_t i = 0; i < 36; ++i) dram_input_p3[i] = 0.0f;
    ...
    for (size_t i = 0; i < 36; ++i) dram_weight_p3[i] = 0.0f;
    for (size_t i = 0; i < 16; ++i) dram_output_p0[i] = 0.0f;
    for (size_t i = 0; i < 16; ++i) dram_output_p1[i] = 0.0f;
    for (size_t i = 0; i < 16; ++i) dram_output_p2[i] = 0.0f;
    for (size_t i = 0; i < 16; ++i) dram_output_p3[i] = 0.0f;
    ...
    m_param_alloc(6, 36 * sizeof(DTYPE));
    m_param_alloc(7, 36 * sizeof(DTYPE));
    m_param_alloc(8, 16 * sizeof(DTYPE));
    m_param_alloc(9, 16 * sizeof(DTYPE));
    m_param_alloc(10, 16 * sizeof(DTYPE));
    m_param_alloc(11, 16 * sizeof(DTYPE));

#endif

    printf("Running HLS top_level function...\n");
    top_level(dram_input_p0, dram_input_p1, dram_input_p2, dram_input_p3, dram_weight_p0, dram_weight_p1, dram_weight_p2, dram_weight_p3, dram_output_p0, dram_output_p1, dram_output_p2, dram_output_p3);
      ...
      for (int p = 0; p < 4; ++p)
        for (int q = 0; q < 4; ++q) {
          ...
          int output_dram_offset = (output_filter_tile * 4 + output_row_tile) * 2 + output_col_tile;
                    int output_port_index = output_bank_index % 4;
          int safe_dram_offset = output_dram_offset * 2 + output_bank_index / 4;
