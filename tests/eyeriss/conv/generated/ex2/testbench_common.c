      ...
      for (int c = 0; c < 4; ++c)
        for (int r = 0; r < 3; ++r)
          for (int s = 0; s < 3; ++s)
            for (int p = 0; p < 8; ++p)
              for (int q = 0; q < 8; ++q) {
                int out_idx = m*8*8 + p*8 + q;
                int w_idx   = m*4*3*3 + c*3*3 + r*3 + s;
                int in_idx  = c*10*10 + (p+r)*10 + (q+s);
...
int main() {
    size_t in_full_e  = 400;
    size_t w_full_e   = 288;
    size_t out_full_e = 512;
    ...
    // Input ports
    DTYPE *dram_input_p0  = (DTYPE*)malloc(100 * sizeof(DTYPE));
    DTYPE *dram_input_p1  = (DTYPE*)malloc(100 * sizeof(DTYPE));
    DTYPE *dram_input_p2  = (DTYPE*)malloc(100 * sizeof(DTYPE));
    DTYPE *dram_input_p3  = (DTYPE*)malloc(100 * sizeof(DTYPE));
    ...
    // Weight ports
    DTYPE *dram_weight_p0 = (DTYPE*)malloc(72 * sizeof(DTYPE));
    DTYPE *dram_weight_p1 = (DTYPE*)malloc(72 * sizeof(DTYPE));
    DTYPE *dram_weight_p2 = (DTYPE*)malloc(72 * sizeof(DTYPE));
    DTYPE *dram_weight_p3 = (DTYPE*)malloc(72 * sizeof(DTYPE));
    ...
    // Output ports
    DTYPE *dram_output_p0 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p0) { printf("Output port 0 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p1 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p1) { printf("Output port 1 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p2 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p2) { printf("Output port 2 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p3 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p3) { printf("Output port 3 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p4 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p4) { printf("Output port 4 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p5 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p5) { printf("Output port 5 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p6 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p6) { printf("Output port 6 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p7 = (DTYPE*)malloc(64 * sizeof(DTYPE));
    if (!dram_output_p7) { printf("Output port 7 alloc failed!\n"); return -1; }


    // Zero all ports
    for (size_t i = 0; i < 100; ++i) dram_input_p0[i] = 0.0f;
    for (size_t i = 0; i < 100; ++i) dram_input_p1[i] = 0.0f;
    for (size_t i = 0; i < 100; ++i) dram_input_p2[i] = 0.0f;
    for (size_t i = 0; i < 100; ++i) dram_input_p3[i] = 0.0f;
    for (size_t i = 0; i < 72; ++i) dram_weight_p0[i] = 0.0f;
    for (size_t i = 0; i < 72; ++i) dram_weight_p1[i] = 0.0f;
    for (size_t i = 0; i < 72; ++i) dram_weight_p2[i] = 0.0f;
    for (size_t i = 0; i < 72; ++i) dram_weight_p3[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p0[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p1[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p2[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p3[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p4[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p5[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p6[i] = 0.0f;
    for (size_t i = 0; i < 64; ++i) dram_output_p7[i] = 0.0f;
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        for (int y = 0; y < 10; ++y) {
            for (int x = 0; x < 10; ++x) {
                int full_idx = c*10*10 + y*10 + x;
                int b_idx    = blk*10*10 + y*10 + x;
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 100 * sizeof(DTYPE));
    m_param_alloc(1, 100 * sizeof(DTYPE));
    m_param_alloc(2, 100 * sizeof(DTYPE));
    m_param_alloc(3, 100 * sizeof(DTYPE));
    m_param_alloc(4, 72 * sizeof(DTYPE));
    m_param_alloc(5, 72 * sizeof(DTYPE));
    m_param_alloc(6, 72 * sizeof(DTYPE));
    m_param_alloc(7, 72 * sizeof(DTYPE));
    m_param_alloc(8, 64 * sizeof(DTYPE));
    m_param_alloc(9, 64 * sizeof(DTYPE));
    m_param_alloc(10, 64 * sizeof(DTYPE));
    m_param_alloc(11, 64 * sizeof(DTYPE));
    m_param_alloc(12, 64 * sizeof(DTYPE));
    m_param_alloc(13, 64 * sizeof(DTYPE));
    m_param_alloc(14, 64 * sizeof(DTYPE));
    m_param_alloc(15, 64 * sizeof(DTYPE));
    ...
    for (int m = 0; m < 8; ++m)
      for (int p = 0; p < 8; ++p)
        for (int q = 0; q < 8; ++q) {
          ...
          int output_bank_index = sacols_0*2 + sacols_1;
          int output_filter_tile = m / 2;
          int output_col_tile = q / 4;
          int output_row_tile = p;
          int output_dram_offset = (output_filter_tile * 8 + output_row_tile) * 2 + output_col_tile;
