      ...
      for (int c = 0; c < 100; ++c)
        for (int r = 0; r < 16; ++r)
          for (int s = 0; s < 16; ++s)
            for (int p = 0; p < 100; ++p)
              for (int q = 0; q < 100; ++q) {
                int out_idx = m*100*100 + p*100 + q;
                int w_idx   = m*100*16*16 + c*16*16 + r*16 + s;
                int in_idx  = c*115*115 + (p+r)*115 + (q+s);
...
int main() {
    size_t in_full_e  = 1322500;
    size_t w_full_e   = 2560000;
    size_t out_full_e = 1000000;
    ...
    // Input ports
    DTYPE *dram_input_p0  = (DTYPE*)malloc(264500 * sizeof(DTYPE));
    DTYPE *dram_input_p1  = (DTYPE*)malloc(264500 * sizeof(DTYPE));
    DTYPE *dram_input_p2  = (DTYPE*)malloc(264500 * sizeof(DTYPE));
    DTYPE *dram_input_p3  = (DTYPE*)malloc(264500 * sizeof(DTYPE));
    DTYPE *dram_input_p4  = (DTYPE*)malloc(264500 * sizeof(DTYPE));

    if (!dram_input_p0 || !dram_input_p1 || !dram_input_p2 || !dram_input_p3 || !dram_input_p4) {
        printf("Input port allocation failed!\n");
        return -1;
    }

    // Weight ports
    DTYPE *dram_weight_p0 = (DTYPE*)malloc(512000 * sizeof(DTYPE));
    DTYPE *dram_weight_p1 = (DTYPE*)malloc(512000 * sizeof(DTYPE));
    DTYPE *dram_weight_p2 = (DTYPE*)malloc(512000 * sizeof(DTYPE));
    DTYPE *dram_weight_p3 = (DTYPE*)malloc(512000 * sizeof(DTYPE));
    DTYPE *dram_weight_p4 = (DTYPE*)malloc(512000 * sizeof(DTYPE));

    if (!dram_weight_p0 || !dram_weight_p1 || !dram_weight_p2 || !dram_weight_p3 || !dram_weight_p4) {
        printf("Weight port allocation failed!\n");
        return -1;
    }

    // Output ports
    DTYPE *dram_output_p0 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p0) { printf("Output port 0 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p1 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p1) { printf("Output port 1 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p2 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p2) { printf("Output port 2 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p3 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p3) { printf("Output port 3 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p4 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p4) { printf("Output port 4 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p5 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p5) { printf("Output port 5 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p6 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p6) { printf("Output port 6 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p7 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p7) { printf("Output port 7 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p8 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p8) { printf("Output port 8 alloc failed!\n"); return -1; }
    DTYPE *dram_output_p9 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_output_p9) { printf("Output port 9 alloc failed!\n"); return -1; }


    // Zero all ports
    for (size_t i = 0; i < 264500; ++i) dram_input_p0[i] = 0.0f;
    for (size_t i = 0; i < 264500; ++i) dram_input_p1[i] = 0.0f;
    for (size_t i = 0; i < 264500; ++i) dram_input_p2[i] = 0.0f;
    for (size_t i = 0; i < 264500; ++i) dram_input_p3[i] = 0.0f;
    for (size_t i = 0; i < 264500; ++i) dram_input_p4[i] = 0.0f;
    for (size_t i = 0; i < 512000; ++i) dram_weight_p0[i] = 0.0f;
    for (size_t i = 0; i < 512000; ++i) dram_weight_p1[i] = 0.0f;
    for (size_t i = 0; i < 512000; ++i) dram_weight_p2[i] = 0.0f;
    for (size_t i = 0; i < 512000; ++i) dram_weight_p3[i] = 0.0f;
    for (size_t i = 0; i < 512000; ++i) dram_weight_p4[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p0[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p1[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p2[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p3[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p4[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p5[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p6[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p7[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p8[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_output_p9[i] = 0.0f;

    // Scatter input into 5 ports by c % 5
    for (int c = 0; c < 100; ++c) {
        int port_index = c % 5;
        int blk        = c / 5;
        for (int y = 0; y < 115; ++y) {
            for (int x = 0; x < 115; ++x) {
                int full_idx = c*115*115 + y*115 + x;
                int b_idx    = blk*115*115 + y*115 + x;
    ...
    // Scatter weights into 5 ports by c % 5
    for (int m = 0; m < 100; ++m) {
      for (int c = 0; c < 100; ++c) {
        int port_index = c % 5;
        int blk        = c / 5;
        for (int r = 0; r < 16; ++r)
          for (int s = 0; s < 16; ++s) {
            int full_idx = m*100*16*16 + c*16*16 + r*16 + s;
            int b_idx    = (m*20 + blk)*16*16 + r*16 + s;
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 264500 * sizeof(DTYPE));
    m_param_alloc(1, 264500 * sizeof(DTYPE));
    m_param_alloc(2, 264500 * sizeof(DTYPE));
    m_param_alloc(3, 264500 * sizeof(DTYPE));
    m_param_alloc(4, 264500 * sizeof(DTYPE));
    m_param_alloc(5, 512000 * sizeof(DTYPE));
    m_param_alloc(6, 512000 * sizeof(DTYPE));
    m_param_alloc(7, 512000 * sizeof(DTYPE));
    m_param_alloc(8, 512000 * sizeof(DTYPE));
    m_param_alloc(9, 512000 * sizeof(DTYPE));
    m_param_alloc(10, 100000 * sizeof(DTYPE));
    m_param_alloc(11, 100000 * sizeof(DTYPE));
    m_param_alloc(12, 100000 * sizeof(DTYPE));
    m_param_alloc(13, 100000 * sizeof(DTYPE));
    m_param_alloc(14, 100000 * sizeof(DTYPE));
    m_param_alloc(15, 100000 * sizeof(DTYPE));
    m_param_alloc(16, 100000 * sizeof(DTYPE));
    m_param_alloc(17, 100000 * sizeof(DTYPE));
    m_param_alloc(18, 100000 * sizeof(DTYPE));
    m_param_alloc(19, 100000 * sizeof(DTYPE));

#endif

    printf("Running HLS top_level function...\n");
    top_level(dram_input_p0, dram_input_p1, dram_input_p2, dram_input_p3, dram_input_p4, dram_weight_p0, dram_weight_p1, dram_weight_p2, dram_weight_p3, dram_weight_p4, dram_output_p0, dram_output_p1, dram_output_p2, dram_output_p3, dram_output_p4, dram_output_p5, dram_output_p6, dram_output_p7, dram_output_p8, dram_output_p9);
    ...
    for (int m = 0; m < 100; ++m)
      for (int p = 0; p < 100; ++p)
        for (int q = 0; q < 100; ++q) {
          ...
          int local_filter_index = m % 5;
          int sacols_1 = local_filter_index;
          int local_col_index = q % 2;
          int sacols_0 = local_col_index;
          int output_bank_index = sacols_0*5 + sacols_1;
          int output_filter_tile = m / 5;
          int output_col_tile = q / 2;
          int output_row_tile = p;
          int output_dram_offset = (output_filter_tile * 100 + output_row_tile) * 50 + output_col_tile;
