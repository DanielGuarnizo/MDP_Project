...
int main() {
    ...
    DTYPE *dram_in_b0 = (DTYPE*)malloc(661250 * sizeof(DTYPE));
    DTYPE *dram_in_b1 = (DTYPE*)malloc(661250 * sizeof(DTYPE));
    DTYPE *dram_w_b0  = (DTYPE*)malloc(1280000 * sizeof(DTYPE));
    DTYPE *dram_w_b1  = (DTYPE*)malloc(1280000 * sizeof(DTYPE));
    ...
    // Outputs
    DTYPE *dram_out_b0 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b0) { printf("Out bank 0 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b1 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b1) { printf("Out bank 1 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b2 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b2) { printf("Out bank 2 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b3 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b3) { printf("Out bank 3 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b4 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b4) { printf("Out bank 4 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b5 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b5) { printf("Out bank 5 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b6 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b6) { printf("Out bank 6 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b7 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b7) { printf("Out bank 7 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b8 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b8) { printf("Out bank 8 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b9 = (DTYPE*)malloc(100000 * sizeof(DTYPE));
    if (!dram_out_b9) { printf("Out bank 9 alloc failed!\n"); return -1; }


    // Zero
    for (size_t i = 0; i < 661250; ++i) { dram_in_b0[i]=0.0f; dram_in_b1[i]=0.0f; }
    for (size_t i = 0; i < 1280000;  ++i) { dram_w_b0[i]=0.0f; dram_w_b1[i]=0.0f; }
    for (size_t i = 0; i < 100000; ++i) dram_out_b0[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b1[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b2[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b3[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b4[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b5[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b6[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b7[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b8[i] = 0.0f;
    for (size_t i = 0; i < 100000; ++i) dram_out_b9[i] = 0.0f;
      ...
      for (int c = 0; c < 100; ++c) {
        ...
        for (int r = 0; r < 16; ++r)
          for (int s = 0; s < 16; ++s) {
            ...
            int b_idx    = (m*50 + blk)*16*16 + r*16 + s;
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 661250 * sizeof(DTYPE));
    m_param_alloc(1, 661250 * sizeof(DTYPE));
    m_param_alloc(2, 1280000 * sizeof(DTYPE));
    m_param_alloc(3, 1280000 * sizeof(DTYPE));
    m_param_alloc(4, 100000 * sizeof(DTYPE));
    m_param_alloc(5, 100000 * sizeof(DTYPE));
    m_param_alloc(6, 100000 * sizeof(DTYPE));
    m_param_alloc(7, 100000 * sizeof(DTYPE));
    m_param_alloc(8, 100000 * sizeof(DTYPE));
    m_param_alloc(9, 100000 * sizeof(DTYPE));
    ...
    top_level(dram_in_b0, dram_in_b1, dram_w_b0, dram_w_b1, dram_out_b0, dram_out_b1, dram_out_b2, dram_out_b3, dram_out_b4, dram_out_b5, dram_out_b6, dram_out_b7, dram_out_b8, dram_out_b9);
      ...
      for (int p = 0; p < 100; ++p)
        for (int q = 0; q < 100; ++q) {
          ...
          switch(output_bank_index) {
            ...
            case 8: out_full[out_idx] = dram_out_b8[output_dram_offset]; break;
            case 9: out_full[out_idx] = dram_out_b9[output_dram_offset]; break;
