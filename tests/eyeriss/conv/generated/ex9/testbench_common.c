          ...
          for (int s = 0; s < 6; ++s)
            for (int p = 0; p < 4; ++p)
              for (int q = 0; q < 4; ++q) {
                ...
                int w_idx   = m*4*3*6 + c*3*6 + r*6 + s;
                int in_idx  = c*6*9 + (p+r)*9 + (q+s);
...
int main() {
    ...
    // Input ports
    DTYPE *dram_input_p0  = (DTYPE*)malloc(108 * sizeof(DTYPE));
    DTYPE *dram_input_p1  = (DTYPE*)malloc(108 * sizeof(DTYPE));
    ...
    // Weight ports
    DTYPE *dram_weight_p0 = (DTYPE*)malloc(144 * sizeof(DTYPE));
    DTYPE *dram_weight_p1 = (DTYPE*)malloc(144 * sizeof(DTYPE));
    ...
    // Zero all ports
    for (size_t i = 0; i < 108; ++i) dram_input_p0[i] = 0.0f;
    for (size_t i = 0; i < 108; ++i) dram_input_p1[i] = 0.0f;
    for (size_t i = 0; i < 144; ++i) dram_weight_p0[i] = 0.0f;
    for (size_t i = 0; i < 144; ++i) dram_weight_p1[i] = 0.0f;
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        for (int y = 0; y < 6; ++y) {
            for (int x = 0; x < 9; ++x) {
                int full_idx = c*6*9 + y*9 + x;
                int b_idx    = blk*6*9 + y*9 + x;
    ...
    for (int m = 0; m < 4; ++m) {
      for (int c = 0; c < 4; ++c) {
        ...
        for (int r = 0; r < 3; ++r)
          for (int s = 0; s < 6; ++s) {
            int full_idx = m*4*3*6 + c*3*6 + r*6 + s;
            int b_idx    = (m*2 + blk)*3*6 + r*6 + s;
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 108 * sizeof(DTYPE));
    m_param_alloc(1, 108 * sizeof(DTYPE));
