          ...
          for (int s = 0; s < 5; ++s)
            for (int p = 0; p < 8; ++p)
              for (int q = 0; q < 8; ++q) {
                ...
                int w_idx   = m*8*5*5 + c*5*5 + r*5 + s;
                int in_idx  = c*12*12 + (p+r)*12 + (q+s);
...
int main() {
    size_t in_full_e  = 1152;
    size_t w_full_e   = 1600;
    ...
    // Input ports
    DTYPE *dram_input_p0  = (DTYPE*)malloc(576 * sizeof(DTYPE));
    DTYPE *dram_input_p1  = (DTYPE*)malloc(576 * sizeof(DTYPE));
    ...
    // Weight ports
    DTYPE *dram_weight_p0 = (DTYPE*)malloc(800 * sizeof(DTYPE));
    DTYPE *dram_weight_p1 = (DTYPE*)malloc(800 * sizeof(DTYPE));
    ...
    // Zero all ports
    for (size_t i = 0; i < 576; ++i) dram_input_p0[i] = 0.0f;
    for (size_t i = 0; i < 576; ++i) dram_input_p1[i] = 0.0f;
    for (size_t i = 0; i < 800; ++i) dram_weight_p0[i] = 0.0f;
    for (size_t i = 0; i < 800; ++i) dram_weight_p1[i] = 0.0f;
    ...
    for (int c = 0; c < 8; ++c) {
        ...
        for (int y = 0; y < 12; ++y) {
            for (int x = 0; x < 12; ++x) {
                int full_idx = c*12*12 + y*12 + x;
                int b_idx    = blk*12*12 + y*12 + x;
    ...
    for (int m = 0; m < 8; ++m) {
      for (int c = 0; c < 8; ++c) {
        ...
        for (int r = 0; r < 5; ++r)
          for (int s = 0; s < 5; ++s) {
            int full_idx = m*8*5*5 + c*5*5 + r*5 + s;
            int b_idx    = (m*4 + blk)*5*5 + r*5 + s;
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 576 * sizeof(DTYPE));
    m_param_alloc(1, 576 * sizeof(DTYPE));
    m_param_alloc(2, 800 * sizeof(DTYPE));
    m_param_alloc(3, 800 * sizeof(DTYPE));
    m_param_alloc(4, 64 * sizeof(DTYPE));
    m_param_alloc(5, 64 * sizeof(DTYPE));
    m_param_alloc(6, 64 * sizeof(DTYPE));
    m_param_alloc(7, 64 * sizeof(DTYPE));
    ...
    top_level(dram_input_p0, dram_input_p1, dram_weight_p0, dram_weight_p1, dram_output_p0, dram_output_p1, dram_output_p2, dram_output_p3, dram_output_p4, dram_output_p5, dram_output_p6, dram_output_p7);
    ...
    for (int m = 0; m < 8; ++m)
      for (int p = 0; p < 8; ++p)
        for (int q = 0; q < 8; ++q) {
          ...
          int output_dram_offset = (output_filter_tile * 8 + output_row_tile) * 4 + output_col_tile;
