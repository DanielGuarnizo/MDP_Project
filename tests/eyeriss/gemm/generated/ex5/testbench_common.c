...
int main() {
    ...
    DTYPE *dram_w_b0  = (DTYPE*)malloc(2  * sizeof(DTYPE));
    DTYPE *dram_w_b1  = (DTYPE*)malloc(2  * sizeof(DTYPE));
    ...
    for (size_t i = 0; i < 2;  ++i) { dram_w_b0[i] = 0.0f; dram_w_b1[i] = 0.0f; }
...
#ifdef __BAMBU_SIM__
    m_param_alloc(0, 2 * sizeof(DTYPE));
    m_param_alloc(1, 2 * sizeof(DTYPE));
