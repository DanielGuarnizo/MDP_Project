...
int main() {
    ...
    DTYPE *dram_in_b0 = (DTYPE*)malloc(2 * sizeof(DTYPE));
    DTYPE *dram_in_b1 = (DTYPE*)malloc(2 * sizeof(DTYPE));
    if (!dram_w_b0 || !dram_w_b1 || !dram_in_b0 || !dram_in_b1) {
        printf("Bank alloc failed!\n"); return -1;
    }
    DTYPE *dram_out_b0 = (DTYPE*)malloc(1 * sizeof(DTYPE));
    if (!dram_out_b0) { printf("Out bank 0 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b1 = (DTYPE*)malloc(1 * sizeof(DTYPE));
    if (!dram_out_b1) { printf("Out bank 1 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b2 = (DTYPE*)malloc(1 * sizeof(DTYPE));
    if (!dram_out_b2) { printf("Out bank 2 alloc failed!\n"); return -1; }
    DTYPE *dram_out_b3 = (DTYPE*)malloc(1 * sizeof(DTYPE));
    if (!dram_out_b3) { printf("Out bank 3 alloc failed!\n"); return -1; }

    for (size_t i = 0; i < 8;  ++i) { dram_w_b0[i] = 0.0f; dram_w_b1[i] = 0.0f; }
    for (size_t i = 0; i < 2; ++i) { dram_in_b0[i] = 0.0f; dram_in_b1[i] = 0.0f; }
    for (size_t i = 0; i < 1; ++i) dram_out_b0[i] = 0.0f;
    for (size_t i = 0; i < 1; ++i) dram_out_b1[i] = 0.0f;
    for (size_t i = 0; i < 1; ++i) dram_out_b2[i] = 0.0f;
    for (size_t i = 0; i < 1; ++i) dram_out_b3[i] = 0.0f;
    ...
    for (int k = 0; k < 4; ++k) {
      int k_bank = k & 1, k_blk = k >> 1;
      for (int n = 0; n < 1; ++n) {
        DTYPE v = B_full[k * 1 + n];
        if (k_bank == 0) dram_in_b0[k_blk * 1 + n] = v;
        else             dram_in_b1[k_blk * 1 + n] = v;
      }
    }
    ...
    m_param_alloc(2, 2 * sizeof(DTYPE));
    m_param_alloc(3, 2 * sizeof(DTYPE));
    m_param_alloc(4, 1 * sizeof(DTYPE));
    m_param_alloc(5, 1 * sizeof(DTYPE));
    m_param_alloc(6, 1 * sizeof(DTYPE));
    m_param_alloc(7, 1 * sizeof(DTYPE));
