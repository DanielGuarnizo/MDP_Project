...
int main() {
    ...
    DTYPE *dram_w_b0  = (DTYPE*)malloc(9  * sizeof(DTYPE));
    DTYPE *dram_w_b1  = (DTYPE*)malloc(9  * sizeof(DTYPE));
    DTYPE *dram_in_b0 = (DTYPE*)malloc(21 * sizeof(DTYPE));
    DTYPE *dram_in_b1 = (DTYPE*)malloc(21 * sizeof(DTYPE));
    if (!dram_w_b0 || !dram_w_b1 || !dram_in_b0 || !dram_in_b1) {
        printf("Bank alloc failed!\n"); return -1;
    }
    DTYPE *dram_out_b0 = (DTYPE*)malloc(21 * sizeof(DTYPE));
    if (!dram_out_b0) { printf("Out bank 0 alloc failed!\n"); return -1; }

    for (size_t i = 0; i < 9;  ++i) { dram_w_b0[i] = 0.0f; dram_w_b1[i] = 0.0f; }
    for (size_t i = 0; i < 21; ++i) { dram_in_b0[i] = 0.0f; dram_in_b1[i] = 0.0f; }
    for (size_t i = 0; i < 21; ++i) dram_out_b0[i] = 0.0f;

    // Scatter A[M x K] into 2 weight banks by k%2
    for (int m = 0; m < 3; ++m) {
      for (int k = 0; k < 5; ++k) {
        int k_bank = k & 1, k_blk = k >> 1;
        DTYPE v = A_full[m * 5 + k];
        if (k_bank == 0) dram_w_b0[m * 3 + k_blk] = v;
        else             dram_w_b1[m * 3 + k_blk] = v;
      }
    }
    ...
    for (int k = 0; k < 5; ++k) {
      int k_bank = k & 1, k_blk = k >> 1;
      for (int n = 0; n < 7; ++n) {
        DTYPE v = B_full[k * 7 + n];
        if (k_bank == 0) dram_in_b0[k_blk * 7 + n] = v;
        else             dram_in_b1[k_blk * 7 + n] = v;
      }
    }

#ifdef __BAMBU_SIM__
    m_param_alloc(0, 9 * sizeof(DTYPE));
    m_param_alloc(1, 9 * sizeof(DTYPE));
    m_param_alloc(2, 21 * sizeof(DTYPE));
    m_param_alloc(3, 21 * sizeof(DTYPE));
    m_param_alloc(4, 21 * sizeof(DTYPE));

#endif

    printf("Running HLS top_level function...\n");
    top_level(dram_w_b0, dram_w_b1, dram_in_b0, dram_in_b1, dram_out_b0);
